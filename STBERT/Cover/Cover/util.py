...
def pytorch_Dis(a: Tensor, b: Tensor):
    pdist = nn.PairwiseDistance(p=2)
    output = pdist(a, b)
    return output

def pytorch_DPS(a: Tensor, b: Tensor):
    return (a * b).sum(dim=-1)

def pytorch_Manhattan_Dis(a: Tensor, b: Tensor):
...
def pytorch_BS(a: Tensor, b: Tensor):
    weight_matrix = nn.Parameter(torch.Tensor(512, 512))
    bias = nn.Parameter(torch.Tensor(1))
    nn.init.xavier_uniform_(weight_matrix)
    bias.data.fill_(0)

    intermediate = torch.matmul(a, weight_matrix)
    result = (intermediate * b).sum(dim=-1) + bias
    return result
